...
6. For repos with matching source signals, that same render path can derive supplemental deterministic artifacts such as Python `pydeps` / `pyreverse` diagrams, shell-flow graphs, database-schema graphs, and tooling-integration graphs.
...
- For repos with supported source signals, the generated blueprint also documents the supplemental deterministic sidecars that `archility render` can materialize.
...
- It also derives deterministic supplemental sidecars from detected Python packages/modules, shell scripts, SQL/schema files, and tooling entrypoints.
...
- The supplemental sidecar diagrams are additive introspection assets. They should not be treated as a replacement for the repo-authored architecture blueprint or the paired `repo-architecture` files.
